...
class BmHsFunc032(Bm):
    def __init__(self, d=3, n=64, seed=42, name=None):
        super().__init__(d, n, seed, name)
        self.set_desc("""
            The function 032 from the Hock & Schittkowski collection.
            ...
            F - objective function
                (x[0] + 3 * x[1] + x[2]) ** 2 + 4 * (x[0] - x[1]) ** 2
            C - constraint function
                6 * x[1] + 4 * x[2] - x[0] ** 3 - 3 >= 0
                1 - x[0] - x[1] - x[2] = 0
    ...
    def _constr_batch(self, X):
        c_1 = -1 * (6 * X[:, 1] + 4 * X[:, 2] - X[:, 0] ** 3 - 3)
        c_2 = np.abs(1 - X[:, 0] - X[:, 1] - X[:, 2])
