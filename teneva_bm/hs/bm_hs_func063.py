...
class BmHsFunc063(Bm):
    def __init__(self, d=3, n=64, seed=42, name=None):
        super().__init__(d, n, seed, name)
        self.set_desc("""
            The function 063 from the Hock & Schittkowski collection.
            ...
            F - objective function
                1000 - x[0] ** 2 - 2 * x[1] ** 2 - x[2] ** 2 - x[0] * x[1] - x[0] * x[2]
            C - constraint function
                8 * x[0] + 14 * x[1] + 7 * x[2] - 56 = 0
                x[0] ** 2 + x[1] ** 2 + x[2] ** 2 - 25 = 0
    ...
    def _constr_batch(self, X):
        c_1 = np.abs(8 * X[:, 0] + 14 * X[:, 1] + 7 * X[:, 2] - 56)
        c_2 = np.abs(X[:, 0] ** 2 + X[:, 1] ** 2 + X[:, 2] ** 2 - 25)
