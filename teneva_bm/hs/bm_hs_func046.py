...
class BmHsFunc046(Bm):
    def __init__(self, d=5, n=64, seed=42, name=None):
        super().__init__(d, n, seed, name)
        self.set_desc("""
            The function 046 from the Hock & Schittkowski collection.
    ...
    def _constr_batch(self, X):
        c_1 = np.abs(X[:, 0] ** 2 * X[:, 3] + np.sin(X[:, 3] - X[:, 4]) - 1)
        c_2 = np.abs(X[:, 1] + X[:, 2] ** 4 * X[:, 3] ** 2 - 2)
