...
class BmHsFunc033(Bm):
    def __init__(self, d=3, n=64, seed=42, name=None):
        super().__init__(d, n, seed, name)
        self.set_desc("""
            The function 033 from the Hock & Schittkowski collection.
            ...
            F - objective function
                (x[0] - 1) * (x[0] - 2) * (x[0] - 3) + x[2]
            C - constraint function
                x[2] ** 2 - x[1] ** 2 - x[0] ** 2 >= 0
                x[0] ** 2 + x[1] ** 2 + x[2] ** 2 - 4 >= 0
        ...
        self.set_grid([0, 0, 0], [+10, +10, 5])
        self.set_min(x=[0, np.sqrt(2), np.sqrt(2)], y=np.sqrt(2) - 6)
    ...
    def _constr_batch(self, X):
        c_1 = -1 * (X[:, 2] ** 2 - X[:, 1] ** 2 - X[:, 0] ** 2)
        c_2 = -1 * (X[:, 0] ** 2 + X[:, 1] ** 2 + X[:, 2] ** 2 - 4)
