...
class BmHsFunc079(Bm):
    def __init__(self, d=5, n=64, seed=42, name=None):
        super().__init__(d, n, seed, name)
        self.set_desc("""
            The function 079 from the Hock & Schittkowski collection.
            ...
            F - objective function
                (x[0] - 1) ** 2 + (x[0] - x[1]) ** 2 + (x[1] - x[2]) ** 2 + \
                (x[2] - x[3]) ** 4 + (x[3] - x[4]) ** 4
            C - constraint function
                x[0] + x[1] ** 2 + x[2] ** 3 - 2 - 3 * sqrt(2) = 0
                x[1] - x[2] ** 2 + x[3] + 2 - 2 * sqrt(2) = 0
                x[0] * x[4] - 2 = 0
    ...
    def _constr_batch(self, X):
        c_1 = np.abs(X[:, 0] + X[:, 1] ** 2 + X[:, 2] ** 3 - 2 - 3 * np.sqrt(2))
        c_2 = np.abs(X[:, 1] - X[:, 2] ** 2 + X[:, 3] + 2 - 2 * np.sqrt(2))
        c_3 = np.abs(X[:, 0] * X[:, 4] - 2)
