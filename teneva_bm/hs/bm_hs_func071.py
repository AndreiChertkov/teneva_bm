...
class BmHsFunc071(Bm):
    def __init__(self, d=4, n=64, seed=42, name=None):
        super().__init__(d, n, seed, name)
        self.set_desc("""
            The function 071 from the Hock & Schittkowski collection.
    ...
    def _constr_batch(self, X):
        c_1 = -1 * (X[:, 0] * X[:, 1] * X[:, 2] * X[:, 3] - 25)
        c_2 = np.abs(X[:, 0] ** 2 + X[:, 1] ** 2 + X[:, 2] ** 2 + X[:, 3] ** 2 - 40)
