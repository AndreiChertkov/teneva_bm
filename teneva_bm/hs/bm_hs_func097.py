...
class BmHsFunc097(Bm):
    def __init__(self, d=6, n=64, seed=42, name=None):
        super().__init__(d, n, seed, name)
        self.set_desc("""
            The function 097 from the Hock & Schittkowski collection.
            ...
            C - constraint function
                17.1 * x[0] + 38.2 * x[1] + 204.2 * x[2] + 212.3 * x[3] + 623.4 * x[4] + \
                1495.5 * x[5] - 169 * x[0] * x[2] - 3580 * x[2] * x[4] - 3810 * x[3] * x[4] - \
                18500 * x[3] * x[5] - 24300 * x[4] * x[5] - b[0] >= 0
                ---------------------------------------------------------------------------
                17.9 * x[0] + 36.8 * x[1] + 113.9 * x[2] + 169.7 * x[3] + 337.8 * x[4] + \
                1385.2 * x[5] - 139 * x[0] * x[2] - 2450 * x[3] * x[4] - 16600 * x[3] * x[5] - \
                17200 * x[4] * x[5] - b[1] >= 0
                ---------------------------------------------------------------------------
    ...
    def _constr_batch(self, X):
        c_1 = -1 * (
            17.1 * X[:, 0] + 38.2 * X[:, 1] + 204.2 * X[:, 2] + 212.3 * X[:, 3] + 623.4 * X[:, 4] + \
            1495.5 * X[:, 5] - 169 * X[:, 0] * X[:, 2] - 3580 * X[:, 2] * X[:, 4] - 3810 * X[:, 3] * X[:, 4] - \
            18500 * X[:, 3] * X[:, 5] - 24300 * X[:, 4] * X[:, 5] - self.parameters['b'][0]
        )
        c_2 = -1 * (
            17.9 * X[:, 0] + 36.8 * X[:, 1] + 113.9 * X[:, 2] + 169.7 * X[:, 3] + 337.8 * X[:, 4] + \
            1385.2 * X[:, 5] - 139 * X[:, 0] * X[:, 2] - 2450 * X[:, 3] * X[:, 4] - \
        ...
        c_4 = -1 * (
            159.9 * X[:, 0] - 311 * X[:, 1] + 587 * X[:, 3] + 391 * X[:, 4] + \
            2198 * X[:, 5] - 14000 * X[:, 0] * X[:, 5] - self.parameters['b'][3]
        )
